...
func (s stateMachine) Lookup(data any) (any, error) {
	fmt.Println("in lookup")
	kb, ok := data.([]byte)

	if ! ok {
		return nil, errors.New("Lookup used wrong type")
	}

	k := string(kb)
	return []byte(s.data[k]), nil
}

func (s stateMachine) Update(e sm.Entry) (sm.Result, error) {
	fmt.Println("in update")
	parts := strings.Split(`=`, string(e.Cmd))
...
func (s stateMachine) SaveSnapshot(writer io.Writer, fc sm.ISnapshotFileCollection, done <-chan struct{}) error {
	fmt.Println("in SaveSnapshot")
	jsData, jsErr := json.Marshal(&s.data)
	if jsErr != nil { return jsErr }

	_, ioErr := writer.Write(jsData)

	return ioErr
}

func (s stateMachine) RecoverFromSnapshot(reader io.Reader, fc []sm.SnapshotFile, done <-chan struct{}) error {
	fmt.Println("in RecoverFromSnapshot")
	jsData, ioErr := ioutil.ReadAll(reader)
	if ioErr != nil { return ioErr }

	jsErr := json.Unmarshal(jsData, &s.data)
	if jsErr != nil { return jsErr }

	fmt.Println("data now:", s.data)
...
func NewStateMachine(replicaID uint64, shardID uint64) sm.IStateMachine {
	return &stateMachine{
		data: map[string]string{
			"foo": "bar",
		},
...
func main() {
	...
	if err := nh.StartReplica(initialMembers, *join, NewStateMachine, rc); err != nil {
	...
	raftStopper.RunWorker(func() {
		...
		for {
			...
			case <-ticker.C:
				...
				result, err := nh.SyncRead(ctx, exampleShardID, []byte("foo"))
				cancel()
				if err == nil {
					res := string(result.([]byte))
					fmt.Fprintf(os.Stdout, "res: %s\n", res)
				}
		...
		for {
			...
			case v, ok := <-ch:
				...
				if cmd, addr, replicaID, err := splitMembershipChangeCmd(msg); err == nil {
					...
					_, err := nh.SyncPropose(ctx, cs, []byte("foo=bob"))
					cancel()
					if err != nil {
